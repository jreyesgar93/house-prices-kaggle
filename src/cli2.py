...
@click.command()
@click.option("--houseid", type=int, help="Input the House Id to get prediction",)
@click.option("--restart", type = click.Choice(["yes","no"]), help="Restart and retrain model.")
def main(houseid,restart):
 ...
 #       os.system("rm tmp/clean_data/* tmp/selected_features/* tmp/models/* tmp/split_sets/* ../predictions/* data/* ../model/*")
        os.system("PYTHONPATH=. luigi --module pipeline.task_7_prediction Prediction --idx "+str(houseid)+" --local-scheduler")
        print(os.system("echo 'Your prediction is:' & cat predictions/prediction-id-"+str(houseid)+".txt"),file = sys.stdout)
    else:
        os.system("PYTHONPATH=. luigi --module pipeline.task_7_prediction Prediction --idx "+str(houseid)+" --local-scheduler > /dev/null")
        print(os.system("echo 'Your prediction is:' & cat predictions/prediction-id-"+str(houseid)+".txt"),file = sys.stdout)
